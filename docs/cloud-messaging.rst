...
To target Firebase Installation IDs instead, use ``sendMulticastToFids()``. It is defined on the
``Kreait\Firebase\Contract\MessagingWithMulticast`` interface, which ``Messaging`` implements in
addition to ``Kreait\Firebase\Contract\Messaging``.
...
.. code-block:: php
    ...
    /** @var Kreait\Firebase\Messaging\MulticastSendReport $sendReport **/
    $sendReport = $messaging->sendMulticastToFids($message, $fids);

The same interface provides ``sendMulticastToRegistrationTokens()``, which is what ``sendMulticast()``
defers to. ``sendMulticast()`` is unchanged and keeps treating strings and arrays of strings as
registration tokens; a future major release will deprecate it in favor of the two explicit methods.

.. code-block:: php

    /** @var Kreait\Firebase\Messaging\MulticastSendReport $sendReport **/
    $sendReport = $messaging->sendMulticastToRegistrationTokens($message, $deviceTokens);
